...
任务到达
    ...
    │
    ├── M 模式 — 标准变更？────────────→ /locking
    │         ├── DEFINE + PLAN → dev:senior-developer (opus)
    │         ├── BUILD → dev:full-stack-developer (sonnet)
    │         ├── REVIEW → dev:junior-reviewer (sonnet)
    │         └── FIX → dev:full-stack-developer (sonnet)
    │
    ...
    │
    ├── 仅 DEFINE + PLAN？────────→ dev:senior-developer (opus)
...
LOCKING  (M) → /locking  → planner → full-stack-developer → junior-reviewer → full-stack-developer
