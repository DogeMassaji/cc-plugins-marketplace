...
任务到达
    │
    ├── 需求 → 代码全流程？──────→ /workflow
    │         ├── DEFINE + PLAN                  → senior-developer-planner (opus)
    │         ├── BUILD（前后端分离 - 后端）      → senior-developer-backend (sonnet)
    │         │       └── BUILD（前后端分离 - 前端）→ senior-developer-frontend (sonnet)
    │         ├── BUILD（单体 / 非前后端分离）     → senior-developer (sonnet)
    │         └── REVIEW                         → senior-reviewer (opus)
    │
    ├── 仅 DEFINE + PLAN？────────→ senior-developer-planner (opus)
    ├── 仅 BUILD 后端？──────────→ senior-developer-backend (sonnet)
    ├── 仅 BUILD 前端？──────────→ senior-developer-frontend (sonnet)
    ├── 仅 BUILD 单体？──────────→ senior-developer (sonnet)
    └── 仅 REVIEW？──────────────→ senior-reviewer (opus)
```
...
任务到达
    ...
    ├── 生成后端测试？───────────→ backend-test-generator
    ├── 生成接口文档？───────────→ api-doc-generator
