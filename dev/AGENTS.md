...
| **L** | `/breaking` | >200 LOC | 策划者 → 构建（单体/前后端分离）→ 高级审查者 → 修复 → 重新审查（含 FIX-RE-REVIEW 循环） |
...
任务到达
    ...
    │         ├── BUILD → dev:full-stack-engineer (sonnet)
    │         ├── REVIEW → dev:junior-reviewer (sonnet)
    │         └── FIX → dev:full-stack-engineer (sonnet)
    │
    ├── M 模式 — 标准变更？────────────→ /locking
    ...
    │         ├── BUILD（单体）→ dev:full-stack-engineer (sonnet)
    │         ├── REVIEW → dev:senior-reviewer (opus)
    │         ├── FIX → dev:full-stack-engineer / dev:senior-developer-backend / dev:senior-developer-frontend (sonnet)
    │         └── RE-REVIEW → dev:senior-reviewer (opus)
    │
    ...
    ├── 仅 BUILD 单体？──────────→ dev:full-stack-engineer (sonnet)
    ├── 仅 REVIEW（完整）？──────→ dev:senior-reviewer (opus)
...
```
POPPING  (S) → /popping  → full-stack-engineer → junior-reviewer → full-stack-engineer
LOCKING  (M) → /locking  → planner → full-stack-engineer → junior-reviewer → full-stack-engineer
BREAKING (L) → /breaking → planner → build(单体/fullstack) → senior-reviewer → fix → re-review
