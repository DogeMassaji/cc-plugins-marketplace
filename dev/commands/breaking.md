...
| 5 | `dev:senior-engineer`（第一轮） | `opus` | REVIEW → 标注 TODO 状态 → 生成修复清单 |
| 6 | 对应 dev agent | `sonnet` | FIX — 按修复清单逐项修复 |
| 7 | `dev:senior-engineer`（重新审查） | `opus` | RE-REVIEW → 验证修复 → 更新 TODO 状态 |
...
## 步骤 5——启动 dev:senior-engineer（第一轮审查）
...
再次启动 `dev:senior-engineer` subagent，显式指定 `model: "opus"`，传入：
- `REVIEW.md` 路径（包含修复清单）
- 模式标记：`re-review`
...
步骤 5 (第一轮审查): Agent(description="审查代码", subagent_type="dev:senior-engineer", model="opus", prompt="...")
...
步骤 7 (重新审查):   Agent(description="重新审查", subagent_type="dev:senior-engineer", model="opus", prompt="[re-review] 检查修复: REVIEW.md 路径为 .artifacts/.../REVIEW.md")
```
