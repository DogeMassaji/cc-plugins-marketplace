...
| 2 | `dev:full-stack-engineer` | `sonnet` | BUILD |
| 3 | `dev:junior-reviewer` | `sonnet` | REVIEW |
| 4 | `dev:full-stack-engineer` | `sonnet` | FIX — 按修复清单逐项修复 |
...
## 步骤 1——启动 dev:senior-developer-planner（opus）

启动 `dev:senior-developer-planner` subagent，传入用户的需求描述，显式指定 `model: "opus"`。
...
## 步骤 2——启动 dev:full-stack-engineer（sonnet）
...
## 步骤 4——启动 dev:full-stack-engineer 修复（sonnet）

审查完成后，再次启动 `dev:full-stack-engineer` subagent，显式指定 `model: "sonnet"`，传入：
- `review.md` 路径（包含修复清单）
- `todo.md` 路径
...
步骤 2: Agent(description="实现代码", subagent_type="dev:full-stack-engineer", model="sonnet", prompt="读取 .artifacts/.../plan.md 和 .artifacts/.../todo.md，逐任务实现并提交")
...
步骤 4: Agent(description="修复审查问题", subagent_type="dev:full-stack-engineer", model="sonnet", prompt="审查反馈修复: 读取 .artifacts/.../review.md，逐项修复并提交")
```
