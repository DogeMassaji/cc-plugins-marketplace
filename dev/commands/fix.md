---
description: Fix 流水线——从问题描述到修复验证：全栈工程师 (sonnet) → 全栈工程师修复 (sonnet) → 初级审查者 (sonnet)，DIAGNOSE → FIX → REVIEW 单轮
---
...
等待 subagent 完成。向用户展示诊断结果。

---

## 步骤 2——启动 dev:full-stack-developer 修复（sonnet）

诊断完成后，再次启动 `dev:full-stack-developer` subagent，显式指定 `model: "sonnet"`，传入：
- `DIAGNOSE.md` 路径
...
步骤 2: Agent(description="修复代码", subagent_type="dev:full-stack-developer", model="sonnet", prompt="按诊断结果修复: 读取 .artifacts/.../DIAGNOSE.md，逐项修复并提交")
