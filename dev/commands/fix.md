...
- 认可 → - 用 `dev:git-commit` 提交，message：`docs(dev): diagnose docs for <任务简述>`，进步骤 2
- 补充调整 → 填入步骤 2 prompt
- 不认可 → 自行决定，可终止

保持生命周期，等 DIAGNOSE 完成再进行 FIX 等后续阶段。

---

## 步骤 2——修复

收到用户确认后，起步骤 1 的 `dev:full-stack-developer` subagent，传：
- `DIAGNOSE.md` 路径
- 用户补充信息（如有）
