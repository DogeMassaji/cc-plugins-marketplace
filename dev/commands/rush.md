...
| 2 | REVIEW | 执行 `dev:review`，五轴审查 → 输出修复建议 |
| 3 | FIX | 起 `dev:full-stack-developer` subagent，执行 `dev:fix` |
...
Agent 执行 `dev:fix`：读修复清单 → 按严重级排序 → 逐项修复+验证+提交（commit 标注 `#review`）→ 单轮结束。
...
2. 任一 subagent 失败则停流水线
3. 审查产物写入 `.artifacts/<yyyymmdd>/<任务简述>/REVIEW.md`
4. 单轮审查 + 单轮修复——无重新审查循环
5. 无策划产物——不生成 SPEC.md / PLAN.md / TODO.md
