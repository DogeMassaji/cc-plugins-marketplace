...
| 步骤 | 阶段 | 方式 |
|------|------|------|
| 1 | DEFINE + PLAN | 起 `dev:planner` subagent，执行 `dev:spec` → `dev:plan`，产出 SPEC.md + PLAN.md + TODO.md |
...
| 4 | REVIEW | 起 `dev:planner` subagent，执行 `dev:review`，产出 REVIEW.md + 更新 TODO.md |
| 5 | FIX | 起对应 dev subagent，执行 `dev:diagnose` |
...
起对应 subagent，指定 `model: "sonnet"`。Agent 执行 `dev:diagnose`：读 REVIEW.md → 按严重级排序 → 逐项修复+验证+提交 → 单轮结束。
...
| 用户意图 | 动作 |
|----------|------|
| "从 plan 开始" | 起 `dev:planner` subagent 执行 `dev:plan` |
...
3. **后端先于前端**——分离路径中，2-B 完成且产出 API.md 后才能 3-B
4. **接口文档是契约**——API.md 是前后端接口契约，前端严格对照
5. 同路径内严格串行——失败即停
6. 修复清单即契约——REVIEW.md `- [ ]` 是修复唯一依据，不扩大范围
7. 所有产物写入 `.artifacts/<yyyymmdd>/<任务简述>/`
