---
description: Ramble 流水线（L 模式）——大型变更完整交付：策划者 (opus) → 用户确认 → 按单体/前后端分离两条路径执行构建 → 审查者 (opus) → 修复
---

Ramble — 漫步稳进。L 模式（大型变更 >200 LOC）完整流水线。调用专职 subagent 完成全部阶段，审查后自动修复，单轮即结束。
...
## 步骤 1——启动 dev:senior-engineer 策划（opus）
...
### 步骤 2-A——启动 dev:full-stack-developer（sonnet）
...
等待 subagent 完成。向用户展示构建结果。跳至**步骤 4**。

---

## 前后端分离路径

### 步骤 2-B——启动 dev:backend-developer（sonnet）
...
### 步骤 3-B——启动 dev:frontend-developer（sonnet）
...
--- 展示策划结果（SPEC.md + PLAN.md + TODO.md），等待用户确认 ---

单体路径：
  步骤 2-A: Agent(description="实现代码", subagent_type="dev:full-stack-developer", model="sonnet", prompt="...")

前后端分离路径：
  步骤 2-B: Agent(description="实现后端", subagent_type="dev:backend-developer", model="sonnet", prompt="...")
  步骤 3-B: Agent(description="实现前端", subagent_type="dev:frontend-developer", model="sonnet", prompt="...")

步骤 4 (审查):   Agent(description="审查代码", subagent_type="dev:senior-engineer", model="opus", prompt="...")
步骤 5 (修复):   Agent(description="修复审查问题", subagent_type="dev:full-stack-developer", model="sonnet", prompt="审查反馈修复: 读取 .artifacts/.../REVIEW.md，逐项修复并提交")
```
...
7. 产物驱动——subagent 之间通过文件产物传递信息，不依赖上下文记忆
8. 任一 subagent 失败则停止流水线，不得继续
9. 所有产物写入 `.artifacts/<yyyymmdd>/<任务简述>/`，不得散落在项目根目录
